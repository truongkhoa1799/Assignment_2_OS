...
static void * timer_routine(void * args) {
	while (!timer_stop) {
		/*check whether n events are finish or not. if fsh == n events break*/
		int fsh = 0;
		/*check each event is done or not to turn to next slot*/
		int event = 0;
		...
		// must place the displace time at this line
